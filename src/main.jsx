    ...
    <>
      ...
      <header className="site-header">
        ...
        <nav className={menuOpen ? 'nav nav--open' : 'nav'}>
          <button onClick={() => scrollTo('work')}>Work</button><button onClick={() => scrollTo('studio')}>Studio</button><button onClick={() => scrollTo('journal')}>Process</button><button onClick={() => setBookingOpen(true)} className="nav__book">Book a session <ArrowUpRight size={15} /></button><button onClick={() => scrollTo('studio')} className="nav__location"><MapPin size={15} /> Malkajgiri, Secunderabad</button>
        </nav>
      ...
      <main>
        <section id="top" className="hero" ref={heroRef} onMouseMove={tiltHero}>
          ...
          <div className="hero__content">
            <p className="eyebrow hero__location"><span /> Malkajgiri · Secunderabad</p>
            <h1>Street<br /><em>ink.</em></h1>
            <div className="hero__bottom"><p>Bold designs. Real stories.<br />No limits.</p><button className="circle-link" onClick={() => scrollTo('work')} aria-label="Explore our work"><ArrowDownRight /></button></div>
          </div>
          <div className="hero__tool-scene" aria-hidden="true">
            <div className="hero__machine"><img src="/images/tattoo-machine.png" alt="Floating tattoo machine" /></div>
            <div className="ink-bottle"><img src="/images/ink-bottle.png" alt="Black ink bottle" /></div>
            <div className="needle-cartridge"><img src="/images/needle-cartridge.png" alt="Tattoo needle cartridge" /></div>
          </div>
        ...
        <section id="studio" className="studio section-pad scroll-3d"><div className="studio__visual"><img src="https://images.unsplash.com/photo-1611501275019-9b5cda994e8d?auto=format&fit=crop&w=1600&q=90" alt="Tattoo artist working on a client in a dark studio" loading="lazy" /><span className="studio__visual-label">IN THE CHAIR / MALKajGIRI</span><div className="studio__stamp">IK<br /><span>STUDIO</span></div></div><div className="studio__copy"><p className="eyebrow">04 / The ritual</p><h2>No rules.<br />Just <em>respect.</em></h2><p>Come in, slow down, and make something that belongs to you. Our studio is a considered, hygienic space in Malkajgiri for custom tattoos, cover-ups, and precise piercings that become part of your personal language.</p><div className="studio__services"><span>Custom tattoos</span><span>Cover-ups</span><span>Helix piercing</span><span>Fine-line work</span></div><div className="studio__details"><span><MapPin size={17} /> Malkajgiri, Secunderabad</span><span><CalendarDays size={17} /> Daily · 10:30 — 21:00</span></div><button className="text-link" onClick={() => setBookingOpen(true)}>Book your consultation <ArrowUpRight size={16} /></button></div></section>

        <section id="journal" className="journey scroll-3d"><div className="journey__intro section-pad"><p className="eyebrow">05 / The journey</p><h2>Idea to<br />healed <em>skin.</em></h2><p>Four stages. No shortcuts. Every tattoo that leaves this studio goes through all of them.</p></div><div className="process-track">{process.map(([number, title, copy]) => <article key={number} className="process-card"><span>{number}</span><div><h3>{title}</h3><p>{copy}</p></div><ArrowUpRight /></article>)}</div></section>

        <section className="ink-section scroll-3d"><div className="ink-section__surface"><div className="ink-section__ripples" /></div><div className="ink-section__copy"><p className="eyebrow">06 / The belief</p><h2>Every drop<br />has <em>meaning.</em></h2><p>We don't chase trends. We make marks that become part of the way you move through the world.</p></div><span className="ink-section__coordinates">17°28' N / 78°33' E</span></section>

        <section id="contact" className="contact section-pad scroll-3d"><div><p className="eyebrow">07 / Begin here</p><h2>Ready to<br />get <em>inked?</em></h2></div><div className="contact__side"><p>Tell us what you've been carrying. We'll help you make it permanent.</p><button className="button button--light" onClick={() => setBookingOpen(true)}>Book appointment <ArrowUpRight size={17} /></button><a href="tel:08885553460" className="contact__phone"><Phone size={15} /> 088855 53460</a></div></section>
      </main>

      <footer className="footer"><div className="footer__top"><div className="footer__logo">INK KING<small>TATTOOZ</small></div><p>Bold designs. Real stories.<br />No limits.</p><a href="https://www.instagram.com/inkking_tattooz/" target="_blank" rel="noreferrer"><ExternalLink size={18} /> @inkking_tattooz</a></div><div className="footer__bottom"><span>© 2024 Ink King Tattooz</span><span>Bold designs. Real stories. No limits.</span><button onClick={() => scrollTo('top')}>Back to top <ArrowUpRight size={14} /></button></div></footer>

      <div className="mobile-cta" role="region" aria-label="Quick booking">
        <a href="tel:08885553460" className="mobile-cta__call" aria-label="Call the studio"><Phone size={18} /></a>
        <button className="mobile-cta__book" onClick={() => setBookingOpen(true)}>Book appointment <ArrowUpRight size={16} /></button>
      </div>
